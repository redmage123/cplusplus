...
// Paired.  You are only allowed to retrieve the promise's future once.
    ...
    int delay;
    public:
        TravelAgent(std::string agentName, int delay) : agentName(agentName),eventualPrice(std::async(std::launch::async, price.get_future())), delay(delay) {
        }
...
*/
    for (auto& agent : agents) {
        std::cout << agent.getName() << " want " << agent.getPrice2() << std::endl;
    }
